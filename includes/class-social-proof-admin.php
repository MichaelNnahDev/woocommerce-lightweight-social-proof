...
class WCLSP_Social_Proof_Admin {
    ...
    public static function register_settings() {
        ...
        add_settings_field( 'bg_color', __( 'Background Color', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_color' ), 'wclsp-settings', 'wclsp_style_section', array( 'id' => 'bg_color', 'tooltip' => 'Background color of the notification card.', 'desc' => 'Default: #151515' ) );
        add_settings_field( 'text_color', __( 'Text Color', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_color' ), 'wclsp-settings', 'wclsp_style_section', array( 'id' => 'text_color', 'tooltip' => 'Base font color for order descriptions.', 'desc' => 'Default: #ffffff' ) );
        add_settings_field( 'accent_color', __( 'Accent Color', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_color' ), 'wclsp-settings', 'wclsp_style_section', array( 'id' => 'accent_color', 'tooltip' => 'Color for customer name and product titles.', 'desc' => 'Default: #D4AF37' ) );
        add_settings_field( 'badge_color', __( 'Verified Badge Color', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_color' ), 'wclsp-settings', 'wclsp_style_section', array( 'id' => 'badge_color', 'tooltip' => 'Color of the Verified check badge.', 'desc' => 'Default: #25D366' ) );
        add_settings_field( 'font_family', __( 'Font Family (CSS)', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_text' ), 'wclsp-settings', 'wclsp_style_section', array( 'id' => 'font_family', 'tooltip' => 'CSS font family stack.', 'desc' => 'e.g. "Mulish", sans-serif or "inherit"' ) );

        // 2. Position & Layout (NEW)
        ...
        add_settings_field( 'position_desktop', __( 'Desktop Position', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_select' ), 'wclsp-settings', 'wclsp_position_section', array(
            ...
            'tooltip' => 'Choose which corner of the screen the popup appears on desktop browsers.',
            'desc'    => 'Select Bottom Left to prevent overlapping floating widgets (e.g. WhatsApp) on the right.',
        ) );
        add_settings_field( 'bottom_offset_desk', __( 'Desktop Bottom Offset (px)', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_number' ), 'wclsp-settings', 'wclsp_position_section', array(
            ...
            'min'     => 0,
            'max'     => 300,
            'tooltip' => 'Distance in pixels from the bottom edge of the desktop browser window.',
            'desc'    => 'Default: 24px.',
        ) );
        add_settings_field( 'bottom_offset_mob', __( 'Mobile Bottom Offset (px)', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_number' ), 'wclsp-settings', 'wclsp_position_section', array(
            ...
            'min'     => 0,
            'max'     => 300,
            'tooltip' => 'Distance from the bottom on mobile devices.',
            'desc'    => 'Increase this (e.g. 70px - 85px) to position the popup cleanly above sticky bottom navigation bars.',
        ) );
        add_settings_field( 'mobile_scale', __( 'Mobile Size Scale (%)', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_number' ), 'wclsp-settings', 'wclsp_position_section', array(
            ...
            'min'     => 60,
            'max'     => 110,
            'tooltip' => 'Scale percentage for mobile screens.',
            'desc'    => 'Recommended: 85% to 90% for a sleek, non-intrusive card on smartphones.',
        ) );
        ...
        add_settings_field( 'initial_delay', __( 'Initial Delay (seconds)', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_number' ), 'wclsp-settings', 'wclsp_behavior_section', array( 'id' => 'initial_delay', 'min' => 1, 'max' => 60, 'tooltip' => 'Delay before first popup shows.', 'desc' => 'Default: 6 seconds.' ) );
        add_settings_field( 'display_duration', __( 'Display Duration (seconds)', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_number' ), 'wclsp-settings', 'wclsp_behavior_section', array( 'id' => 'display_duration', 'min' => 2, 'max' => 30, 'tooltip' => 'How long each popup stays on screen.', 'desc' => 'Default: 6 seconds.' ) );
        add_settings_field( 'min_interval', __( 'Minimum Interval (seconds)', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_number' ), 'wclsp-settings', 'wclsp_behavior_section', array( 'id' => 'min_interval', 'min' => 5, 'max' => 300, 'tooltip' => 'Minimum pause between consecutive popups.', 'desc' => 'Default: 15 seconds.' ) );
        add_settings_field( 'max_interval', __( 'Maximum Interval (seconds)', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_number' ), 'wclsp-settings', 'wclsp_behavior_section', array( 'id' => 'max_interval', 'min' => 5, 'max' => 300, 'tooltip' => 'Maximum pause between consecutive popups.', 'desc' => 'Default: 30 seconds.' ) );
        ...
        add_settings_field( 'order_hours', __( 'Order History Scope (hours)', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_number' ), 'wclsp-settings', 'wclsp_query_section', array( 'id' => 'order_hours', 'min' => 1, 'max' => 720, 'tooltip' => 'Hours of past orders to pull.', 'desc' => 'Default: 48 hours.' ) );
        add_settings_field( 'cache_minutes', __( 'Transient Cache Lifetime (minutes)', 'wc-lightweight-social-proof' ), array( __CLASS__, 'field_number' ), 'wclsp-settings', 'wclsp_query_section', array( 'id' => 'cache_minutes', 'min' => 1, 'max' => 120, 'tooltip' => 'Cache duration in minutes.', 'desc' => 'Default: 5 minutes.' ) );
    }
    ...
    public static function field_text( $args ) {
        ...
        echo '<input type="text" name="wclsp_settings[' . $id . ']" value="' . $val . '" class="regular-text" style="vertical-align:middle;">';
        self::render_tooltip( $tooltip );
        if ( ! empty( $desc ) ) {
            echo '<p class="description" style="margin-top:4px; font-size:12px; color:#646970;">' . esc_html( $desc ) . '</p>';
        }
    ...
    public static function field_number( $args ) {
        ...
        echo '<input type="number" name="wclsp_settings[' . $id . ']" value="' . $val . '" class="small-text" style="vertical-align:middle;"' . $min . $max . '>';
        self::render_tooltip( $tooltip );
        if ( ! empty( $desc ) ) {
            echo '<p class="description" style="margin-top:4px; font-size:12px; color:#646970;">' . esc_html( $desc ) . '</p>';
        }
    ...
    public static function render_settings_page() {
        ...
        <div class="wrap">
            ...
            <p class="description"><?php esc_html_e( 'Customize positioning, layout offsets, styling, and timing parameters.', 'wc-lightweight-social-proof' ); ?></p>
